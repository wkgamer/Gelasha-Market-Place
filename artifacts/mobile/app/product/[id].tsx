import { ArrowLeft, Heart, Star, CheckCircle, XCircle, ShoppingBag, Plus, Minus } from "lucide-react-native";
...
export default function ProductDetailScreen() {
  ...
  async function handlePurchase() {
    ...
    try {
      ...
      if (res.ok) {
        setPurchased(true);
        const variantSummary = selectedVariantList.map((v) => v.name).join(", ");
        if (Platform.OS !== "web") {
          Alert.alert(
            "Order Placed!",
            `Your order for ${product.name}${variantSummary ? ` (${variantSummary})` : ""} × ${quantity} has been confirmed.`,
            [
  ...
  function getAllImages(): string[] {
    ...
    const variantImgs = (product.variants || []).filter((v) => v.image && !base.includes(v.image)).map((v) => v.image!);
  ...
  return (
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: bottomPad + 130 }}>
        <View style={styles.imageSection}>
          ...
          <View style={[styles.imageControls, { top: topPad + 12 }]}>
            ...
            <Pressable style={styles.controlBtn} onPress={toggleLike}>
              <Heart size={22} color={liked ? Colors.light.error : Colors.light.text} fill={liked ? Colors.light.error : "none"} />
            </Pressable>
          </View>
          ...
          <View style={styles.bottomBarInner}>
            <View style={styles.quantityRow}>
              ...
              <Pressable
                ...
                disabled={quantity <= 1}
              >
                <Minus size={16} color={quantity <= 1 ? Colors.light.textMuted : Colors.light.tint} />
              </Pressable>
...
const styles = StyleSheet.create({
  ...
  imageControls: { position: "absolute", left: 0, right: 0, flexDirection: "row", justifyContent: "space-between", paddingHorizontal: 16, zIndex: 10 },
  controlBtn: { width: 42, height: 42, borderRadius: 21, backgroundColor: "rgba(255,255,255,0.92)", alignItems: "center", justifyContent: "center", shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 },
  imageDots: { position: "absolute", bottom: 10, left: 0, right: 0, flexDirection: "row", justifyContent: "center", gap: 5 },
  ...
  categoryTag: { backgroundColor: Colors.light.tintUltraLight, borderRadius: 6, paddingHorizontal: 10, paddingVertical: 4 },
  ...
  variantSelectedLabel: { fontSize: 13, fontFamily: "Inter_500Medium", color: Colors.light.tintDark, backgroundColor: Colors.light.tintUltraLight, borderRadius: 8, paddingHorizontal: 8, paddingVertical: 2 },
  variantGrid: { flexDirection: "row", flexWrap: "wrap", gap: 10 },
  variantBtn: { flexDirection: "row", alignItems: "center", gap: 8, borderWidth: 1.5, borderColor: Colors.light.border, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 10, backgroundColor: "#fff" },
  variantBtnActive: { borderColor: Colors.light.tint, backgroundColor: Colors.light.tintUltraLight, borderWidth: 2 },
  ...
  selectionSummary: { flexDirection: "row", alignItems: "flex-start", gap: 8, backgroundColor: Colors.light.tintUltraLight, borderRadius: 10, padding: 12, borderWidth: 1, borderColor: Colors.light.tintLight },
  selectionSummaryText: { flex: 1, fontSize: 13, fontFamily: "Inter_500Medium", color: Colors.light.tintDark, lineHeight: 20 },

  stockRow: { flexDirection: "row", alignItems: "center", gap: 8, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 10 },
  ...
  bottomBar: { position: "absolute", bottom: 0, left: 0, right: 0, backgroundColor: "#fff", paddingHorizontal: 18, paddingTop: 12, borderTopWidth: 1, borderTopColor: Colors.light.borderLight, shadowColor: "#000", shadowOffset: { width: 0, height: -4 }, shadowOpacity: 0.08, shadowRadius: 12, elevation: 10 },
  bottomBarInner: { gap: 10 },
  quantityRow: { flexDirection: "row", alignItems: "center", gap: 12 },
  quantityLabel: { fontSize: 14, fontFamily: "Inter_600SemiBold", color: Colors.light.text, marginRight: 4 },
  qtyBtn: { width: 36, height: 36, borderRadius: 10, borderWidth: 1.5, borderColor: Colors.light.tint, alignItems: "center", justifyContent: "center", backgroundColor: Colors.light.tintUltraLight },
  qtyBtnDisabled: { borderColor: Colors.light.border, backgroundColor: Colors.light.backgroundSecondary },
  qtyValue: { fontSize: 18, fontFamily: "Inter_700Bold", color: Colors.light.text, minWidth: 32, textAlign: "center" },
  purchaseBtn: { backgroundColor: Colors.light.tint, borderRadius: 14, paddingVertical: 15, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10, shadowColor: Colors.light.tint, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.35, shadowRadius: 12, elevation: 8 },
  ...
  purchasedBanner: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10, backgroundColor: "#F0FDF4", borderRadius: 14, paddingVertical: 15, borderWidth: 1.5, borderColor: "#BBF7D0" },
